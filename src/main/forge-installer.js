'use strict';
/**
 * ForgeInstaller — встановлює Forge 1.20.1-47.4.10 БЕЗ запуску installer.jar
 *
 * Forge installer.jar — це звичайний ZIP-архів, всередині якого:
 *   install_profile.json  — список процесорів та залежностей
 *   version.json          — версійний JSON для запускача
 *   maven/                — вбудовані бібліотеки
 *
 * Ми робимо те, що робить Prism/MultiMC:
 *   1. Завантажуємо ванільний Minecraft + бібліотеки + асети
 *   2. Завантажуємо forge-installer.jar
 *   3. Розпаковуємо його (це ZIP)
 *   4. Копіюємо version.json і вбудовані бібліотеки
 *   5. Завантажуємо всі залежності з install_profile.json
 *   6. Запускаємо кожен processor з install_profile.processors
 *
 * Переваги: жодних зверень до maven.minecraftforge.net під час "установки",
 * все завантажується заздалегідь нашим кодом (з retry, прогресом тощо).
 */
...
// Maven central та Forge Maven — для завантаження залежностей
const MAVEN_REPOS = [
...
class ForgeInstaller extends EventEmitter {
  ...
  isInstalled() {
    // version.json alone is NOT enough — forge-client.jar must also exist
    // (it's created by forge processors, not by copying)
  ...
  async install() {
    ...
    try {
      // 1. Ванільний Minecraft
      const vanillaJson = await this._ensureVanilla();

      // 2. Ігрові ресурси
      await this._ensureAssets(vanillaJson);

      // 3. Завантаження installer.jar
      this._status('Завантаження Forge installer…', 2, 5);
      await this._downloadInstaller();

      // 4. Розпакування installer.jar → читаємо JSON всередині
      this._status('Розпакування Forge installer…', 3, 5);
      const { installProfile, forgeVersionJson } = await this._extractInstaller();

      // 5. Завантажуємо бібліотеки з ОБОХ JSON-ів перед запуском installer
      //    install_profile.json libs → процесори (installertools, binarypatcher тощо)
      //    version.json libs         → bootstraplauncher, securejarhandler, fmlcore тощо
      ...
      // 6. Створюємо launcher_profiles.json — forge installer перевіряє його наявність
      //    і відмовляється встановлюватись якщо файл відсутній
      this._createLauncherProfiles();

      // 7. Запускаємо forge-installer.jar --installClient
      //    Installer сам обробить все: binarypatcher, srg mapping, extra jar
      //    Якщо він впав - не продовжуємо, не рахуємо Forge встановленим
      this._status('Forge processors... (1-5 хвилин, не закривайте)', 5, 6);
      await this._runForgeInstaller();

      // 7. Копіюємо version.json та вбудовані maven/ бібліотеки з installer
      //    Тільки якщо forge-installer успішно завершився!
  ...
  async _ensureVanilla() {
    ...
    // Завантажуємо або читаємо версійний JSON
    let vJson;
    ...
    // Перевіряємо + завантажуємо client.jar
  ...
  async _ensureAssets(vJson) {
    ...
    let done = 0;
    const BATCH = 8; // менший batch — менше паралельних з'єднань
  ...
  // ════════════════════════════════════════════════════════════
  //  Запуск forge-installer.jar — processes patching after deps downloaded
  // ════════════════════════════════════════════════════════════
  _createLauncherProfiles() {
    const profilesPath = path.join(this.gameDir, 'launcher_profiles.json');
    if (fs.existsSync(profilesPath)) return; // вже є — не перезаписуємо
  ...
  _runForgeInstaller() {
    return new Promise((resolve, reject) => {
      ...
      const args = [
        ...
        // Підказуємо forge installer де шукати Maven artifacts (наш libraries/)
        // Це запобігає зверненню до мережі якщо файли вже є
        `-Dmaven.repo.local=${libsDir}`,
        // Forge installer home = папка installer.jar (там є embedded maven repo)
  ...
  // ════════════════════════════════════════════════════════════
  //  4. РОЗПАКУВАННЯ INSTALLER.JAR (це ZIP-архів)
  // ════════════════════════════════════════════════════════════
  async _extractInstaller() {
    // Розпаковуємо через системний інструмент
    await this._unzipFile(this.installerJar, this.installerDir);

    // Читаємо install_profile.json
    ...
    // Читаємо version.json (може бути у корені або в /version.json)
  ...
  async _unzipFile(zipPath, destDir) {
    // Method 1: node-stream-zip (pure Node.js, no system tools needed)
    try {
    ...
    // Method 3: system unzip (Linux/macOS only)
  ...
  // ════════════════════════════════════════════════════════════
  //  5. КОПІЮВАННЯ VERSION.JSON І ВБУДОВАНИХ БІБЛІОТЕК
  // ════════════════════════════════════════════════════════════
  async _setupVersionDir(forgeVersionJson) {
    ...
    // Записуємо version.json
    fs.writeFileSync(vJsonDst, JSON.stringify(forgeVersionJson, null, 2));

    // Копіюємо вбудовані maven/ бібліотеки з installer
  ...
  // ════════════════════════════════════════════════════════════
  //  6. ЗАВАНТАЖЕННЯ ЗАЛЕЖНОСТЕЙ З install_profile.json
  // ════════════════════════════════════════════════════════════
  async _downloadDependencies(libs) {
    // libs — масив бібліотек (може бути з install_profile або version.json)
    if (!Array.isArray(libs)) libs = libs.libraries || [];
    const libsDir = path.join(this.gameDir, 'libraries');

    // Дедуплікуємо по шляху щоб не завантажувати двічі
    const seen = new Set();
    ...
    for (const lib of libs) {
      ...
      // Спробуємо всі можливі URL джерела
      const urls = [];
      if (art?.url)  urls.push(art.url);   // artifact.url
      if (lib.url)   urls.push(lib.url);   // пряме url поле
      if (lib.name) {
  ...
  async _runProcessors(installProfile, vanillaJson) {
    const processors = (installProfile.processors || [])
      .filter(p => {
        // Пропускаємо server-side процесори
    ...
    // Підготовляємо змінні підстановки для processors
  ...
  _runOneProcessor(proc, vars, libsDir, java) {
    return new Promise((resolve, reject) => {
      ...
      // Аргументи процесора — підставляємо змінні
      ...
      child.on('close', code => {
        if (code === 0) {
          ...
          // Деякі процесори виходять з ненульовим кодом але все одно успішні
          // Перевіряємо чи є вихідний файл
          resolve(); // не зупиняємось на помилці одного процесора
        }
      });
  ...
  // "net.minecraftforge:forge:1.20.1-47.4.10:universal" → відносний шлях
  ...
  _dl(url, dest) {
    // Hard timeout: якщо за 20 секунд не завершено — відхиляємо (не вішаємо Promise)
    return new Promise((resolve, reject) => {

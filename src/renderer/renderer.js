...
let consoleLines = [];
let progressTimeout = null;   // таймаут безпеки від вічного завантаження
...
async function startGame() {
  ...
  try {
    // ── 1. Перевірка/встановлення Forge ─────────────────────────────────────
    ...
    // Таймаут безпеки — якщо через 60 с гра не запустилась, знімаємо прогрес
...
async function runModpackUpdate() {
  ...
  if (r.error) {
    ...
    // Оновлюємо вкладку модів якщо вона відкрита
...
// ══════════════════════════════════════════════════════════
//  ВКЛАДКА "МОДИ" — реальний список з папки mods/
// ══════════════════════════════════════════════════════════
async function loadModsTab() {
  ...
  try {
    // Отримуємо реальний список .jar файлів з папки mods/
    const mods = await launcher.listMods();

    $('statModCount').textContent = mods.length;

    // Версія модпаку з маніфесту (якщо є)
    try {
      const check = await launcher.checkModpack();
...
// setupModsTab реєструється в головному DOMContentLoaded вище
...
function appendConsole(line, isErr = false) {
  ...
  // Видаляємо старі рядки якщо їх забагато
...
// ══════════════════════════════════════════════════════════
//  IPC СЛУХАЧІ (встановлюються ОДИН РАЗ)
// ══════════════════════════════════════════════════════════
function setupIPCListeners() {
  ...
  launcher.onClosed(({ code }) => {
    ...
    // Оновлюємо моди після закриття гри
    if ($('tab-mods').classList.contains('active')) loadModsTab();
  });
...
function showModal(title, body) {
  return new Promise(resolve => {
    ...
    const cleanup = result => {
      ...
      // Переприєднуємо кнопки (щоб не накопичувались listeners)
      const ok = $('modalOk');
